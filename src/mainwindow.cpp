...
void MainWindow::zmenPopisAutbobusu(autobusClass *autobus)
{
    ...
    for(int i = 0; i < autobus->zastavkyNaLince->size(); i++){
        if(autobus->zastavkyNaLince->value(i).second + autobus->pocatecniCas < time){
            text << "<font color = 'gray'>";
            text << QTime::fromMSecsSinceStartOfDay((autobus->zastavkyNaLince->value(i).second + autobus->pocatecniCas) * 1000).toString("hh:mm:ss");
